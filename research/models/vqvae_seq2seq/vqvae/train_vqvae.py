...
def train_epoch(
    ...
    return {
        k: (v / n_batches).item() if isinstance(v, torch.Tensor) else v / n_batches
...
def main():
    ...
    if args.cache_dir:
        ...
        print(
            f"Cache: {cached}/{total} samples ready ({cached/total*100:.1f}%) — {'loading from cache' if cached == total else 'will build cache for missing samples'}"
        )
    ...
    for epoch in range(start_epoch, config.max_epochs):
        if not resets_frozen and epoch >= warmdown_epoch:
            ...
            resets_frozen = True
            print(
                f"  Codebook resets disabled (warmdown from epoch {epoch}/{config.max_epochs})"
            )
