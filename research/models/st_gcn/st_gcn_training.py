...
from research.models.st_gcn.data_prep import (
    get_data_loaders,
    ALL_COLUMNS,
    SIGN2INDEX_JSON,
)
...
from research.models.st_gcn.st_gcn_model import ST_GCN_ASL, LightweightST_GCN
    ...
    class ST_GCN_DataLoaderWrapper:
        ...
        def __iter__(self):
            for x, mask, y in self.dataloader:
                ...
                # Reshape to (B, T, N, C)
                x_reshaped = x_pos.reshape(
                    B, T, self.num_landmarks, coords_per_landmark
                )
...
@torch.no_grad()
def test_epoch(
    model: nn.Module, data_loader, criterion: nn.Module
) -> Tuple[float, float]:
    """
...
def main():
    ...
    optimizer = optim.AdamW(
        model.parameters(), lr=1e-3, weight_decay=0.01  # Slightly higher LR for ST-GCN
    )
    ...
    for epoch in range(epochs):
        ...
        train_loss = train_epoch(
            model, train_loader_stgcn, optimizer, criterion, scaler
        )
