...
FACE_GLOBAL_TO_LOCAL = {global_idx: local_idx for local_idx, global_idx in enumerate(SELECTED_FACE_INDICES)}
...
class LandmarkGraph:
    ...
    def __init__(self, include_face=True):
        ...
        self.num_landmarks = 209 if include_face else 75  # face(134) + hands(42) + pose(33)
    ...
    def _get_face_edges(self) -> List[Tuple[int, int]]:
        ...
        left_eye = to_local([33, 133, 160, 159, 158, 157, 173, 144, 145, 153, 154, 155, 156, 246, 7, 163])
        for i in range(len(left_eye)):
            edges.append((left_eye[i], left_eye[(i + 1) % len(left_eye)]))
        
        # Right eye contour (closed loop)
        right_eye = to_local([263, 362, 387, 386, 385, 384, 398, 373, 374, 380, 381, 382, 466, 388, 390, 249])
        ...
        mouth_outer = to_local([61, 146, 91, 181, 84, 17, 314, 405, 321, 375, 
        ...
        mouth_inner = to_local([78, 191, 80, 81, 82, 13, 312, 311, 310, 415,
                                308, 324, 318, 402, 317, 14, 87, 178, 88, 95])
        for i in range(len(mouth_inner)):
            edges.append((mouth_inner[i], mouth_inner[(i + 1) % len(mouth_inner)]))
        
        # Face oval (closed contour)
        face_oval = to_local([10, 338, 297, 332, 284, 251, 389, 356, 454, 323, 361, 288,
        ...
        nose_to_right_eye = (self.face_global_to_local[1], self.face_global_to_local[263])
        left_eye_to_eyebrow = (self.face_global_to_local[33], self.face_global_to_local[70])
        right_eye_to_eyebrow = (self.face_global_to_local[263], self.face_global_to_local[300])
    ...
    def _get_cross_body_edges(self) -> List[Tuple[int, int]]:
        ...
        if self.include_face:
            ...
            edges.extend([
                (left_hand_offset + 0, pose_offset + 15),   # Left hand wrist to pose left wrist
                (right_hand_offset + 0, pose_offset + 16),  # Right hand wrist to pose right wrist
            ])
