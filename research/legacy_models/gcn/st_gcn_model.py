...
from st_gcn_layers import ST_GCN_Block, GraphConvolution, TemporalConvolution
...
class ST_GCN_ASL(nn.Module):
    ...
    def __init__(
        ...
        self.stage1 = nn.ModuleList([
            ST_GCN_Block(64, 64, adj_matrix, temporal_kernel_size, stride=1, dropout=dropout),
            ST_GCN_Block(64, 64, adj_matrix, temporal_kernel_size, stride=1, dropout=dropout),
            ST_GCN_Block(64, 128, adj_matrix, temporal_kernel_size, stride=2, dropout=dropout),
        ])
        
        # Stage 2: Mid-level features (128 channels)
        # Learns: "How do hand shapes combine and transition?"
        self.stage2 = nn.ModuleList([
            ST_GCN_Block(128, 128, adj_matrix, temporal_kernel_size, stride=1, dropout=dropout),
            ST_GCN_Block(128, 128, adj_matrix, temporal_kernel_size, stride=1, dropout=dropout),
            ST_GCN_Block(128, 256, adj_matrix, temporal_kernel_size, stride=2, dropout=dropout),
        ])
        
        # Stage 3: High-level features (256 channels)
        # Learns: "What is the complete sign pattern?"
        self.stage3 = nn.ModuleList([
            ST_GCN_Block(256, 256, adj_matrix, temporal_kernel_size, stride=1, dropout=dropout),
            ST_GCN_Block(256, 256, adj_matrix, temporal_kernel_size, stride=1, dropout=dropout),
            ST_GCN_Block(256, 256, adj_matrix, temporal_kernel_size, stride=1, dropout=dropout),
        ])
...
class LightweightST_GCN(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList([
            ST_GCN_Block(32, 32, adj_matrix, temporal_kernel_size=9, dropout=dropout),
            ST_GCN_Block(32, 64, adj_matrix, temporal_kernel_size=9, stride=2, dropout=dropout),
            ST_GCN_Block(64, 64, adj_matrix, temporal_kernel_size=9, dropout=dropout),
            ST_GCN_Block(64, 128, adj_matrix, temporal_kernel_size=9, stride=2, dropout=dropout),
            ST_GCN_Block(128, 128, adj_matrix, temporal_kernel_size=9, dropout=dropout),
            ST_GCN_Block(128, 128, adj_matrix, temporal_kernel_size=9, dropout=dropout),
        ])
