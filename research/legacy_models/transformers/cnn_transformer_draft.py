...
def normalize_values(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    origins = dataframe[(dataframe["type"] == "pose") & (dataframe["landmark_index"] == 0)].set_index(
        "frame"
    )[axes]
...
def frame_stacked_data(file_path: str) -> np.ndarray:
    ...
    dataframe["uid"] = dataframe["type"].astype("str") + "_" + dataframe["landmark_index"].astype("str")
...
def augment_sample(video_coordinates: np.ndarray, noise_std: float = 3e-3,
                    spatial_shift: float = 2e-2) -> np.ndarray:
    ...
    if np.random.random() > 0.5:
        shift = np.random.uniform(-spatial_shift, spatial_shift,
                                    (1, video_coordinates.shape[1]))
...
def get_data_loaders(use_npz: bool = False) -> Tuple[DataLoader, DataLoader]:
    ...
    """
    if use_npz:
        data = np.load("/kaggle/input/face-data-coordinates/asl_preprocessed.npz", allow_pickle=True)
        ...
        train_videos = [frame_stacked_data(path) for path in train_split["path"].to_list()]
        test_videos = [frame_stacked_data(path) for path in test_split["path"].to_list()]
...
class AdvancedAugmentation:
    ...
    def temporal_cropping(x, mask, min_ratio=0.7, max_ratio=0.95):
        ...
        if crop_len < T:
            ...
            mask_padded = torch.nn.functional.pad(mask_cropped, (0, pad_len), value=False)
